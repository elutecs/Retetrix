...
def condicion(m,c,color):
    ...
    for i in range(3):
        for j in range(3):
            if c.position[i][j]==1:
                if m[c.y+(i)][c.x+j-1]==-1: #antes checkeaba si habian piezas abajo, ahora si estan al costado derecho
                    bandera = True
    if bandera ==True:
...
def revisarMatriz(matriz):
    indice =-1
    for i in reversed(range(50,(len(matriz[0])//2))):
        bandera = False
        for j in range(3,len(matriz)-1):
            if matriz[i][j]==0: bandera = True, print(5)
        if bandera==False:
            print("entra")

            for n in range(i+1,1):
                for j in reversed(range(3,len(matriz)-1)):
...
def principal():
    izquierda = True
    ventana = pygame.display.set_mode((1350,620))
    matriz = crearMatriz(16,53)
    cuadrito,contador,bandera,color = Cuadro(),0,False,[1,1,0]
